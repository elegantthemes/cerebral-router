...
import { isObject, isComplexObject, isSerializable, throwError, forceSerializable, addCerebralStateKey } from './utils';
      ...
      path.reduce(function (currentState, key, index) {
        if (index === path.length - 1) {
          if (!Array.isArray(currentState) && !isObject(currentState)) {
            throwError('The path "' + path.join('.') + '" is invalid. Path: "' + path.slice(0, path.length - 1).join('.') + '" is type of "' + (currentState === null ? 'null' : typeof currentState === 'undefined' ? 'undefined' : _typeof(currentState)) + '"');
          }
